...
$navbar = new AuthNavbar('Reported Donors', '/manager/profile', '/public/images/icons/user.png', false,false);
    ...
    <div id="detail-pane" class="min-w-80 max-w-90 mt-10 min-h-80 d-flex justify-content-center flex-column align-items-center" style="margin-top: 6rem">
        ...
        <div id="card-pane" class="card-pane">
            ...
            foreach ($data as $value) {
                ...
                $cause=$value->getReportCause();
                $NIC=$value->getNIC();
                $action_taken='banned'
//                $branch=$value->getBranchLocation();
                ?>
                <div class="detail-card" id="MO7646" onclick="RedirectDonor('<?= $NIC?>')">
                    <div class="card-image">
                        <img src='<?= $image?>' alt="">
                    </div>
                    <div class="card-body">
                        <div class="card-title"><?= $name ?></div>
                        <div class="card-description"><?= $cause ?></div>
                        <div class="card-description"><?= $NIC ?></div>
                        <div class="card- text-danger"><?= $action_taken ?></div>

<!--                        <div class="card-description">--><?php //= $branch ?><!--</div>-->
                    </div>
                </div>
                <?php
            }
    ...
    <script>
        ...
        function SearchFunction(path='/manager/mngDonors/reportedDonor'){
            const Search=document.getElementById('search');
