...
<!--    <div class="d-flex"><b>Campaign Name</b> : --><?php //=$Campaign->getCampaignName()?><!--</div>-->
<!--    <div class="d-flex"><b>Assigned Task</b> : --><?php //=$Task?><!--</div>-->
<!--</div>-->
<?php
if(!empty($data)):
...
//    foreach ($data as $donor){
//        /* @var $donor Donor*/
//        $card = new NavigationCard($donor->getFirstName().' '.$donor->getLastName(),'/hospital/donor/'.$donor->getNIC(),$donor->getProfilePicture(),'');
//        echo $card->render();
//    }
?>
            ...
            <div id="PersonalDetails" class="d-flex flex-column gap-1 w-90">
                <div class="form-group">
                    <div class="d-flex justify-content-center align-items-center w-100">
                        ...
                        <input type="text" class="form-control w-60 w-60" id="FirstName" placeholder="First Name" value="<?= $data['First_Name']?>" disabled>
                    </div>
                    <div class="d-flex w-100 justify-content-center align-items-center ">
                        <label for="LastName" class="form-label w-40">Last Name</label>
                        <input type="text" class="form-control w-60" id="LastName" placeholder="Last Name" value="<?= $data['Last_Name']?>" disabled>
                    </div>
                </div>
                <div class="form-group">
                    <div class="d-flex w-100 justify-content-center align-items-center ">
                        <label for="NIC" class="form-label w-40">NIC</label>
                        <input type="text" class="form-control w-60" id="NIC" placeholder="National Identity Card No" value="<?= $data['NIC']?>" disabled>
                    </div>
                    <div class="d-flex w-100 justify-content-center align-items-center ">
                        <label for="ContactNo" class="form-label w-40">Contact No</label>
                        <input type="tel" class="form-control w-60" id="ContactNo" placeholder="Contact No" value="<?= $data['Contact_No']?>" disabled>
                    </div>
                </div>
                <div class="form-group">
                    <div class="d-flex w-100 justify-content-center align-items-center ">
                        <label for="Email" class="form-label w-40">Email</label>
                        <input type="email" class=" form-date" id="Email" placeholder="example@mail.com" value="<?= $data['Email']?>" disabled>
                    </div>
<!--                    <div class="d-flex w-100 justify-content-center align-items-center ">-->
<!--                        <label for="Address1" class="form-label w-40">Address1</label>-->
<!--                        <input type="text" class=" form-date" id="Address1" placeholder="No 01" value="--><?php //= $data['']?><!--" disabled>-->
<!--                    </div>-->
                </div>
<!--                <div class="form-group">-->
<!--                    <div class="d-flex w-100 justify-content-center align-items-center ">-->
<!--                        <label for="Email" class="form-label w-40">Address2</label>-->
<!--                        <input type="text" class=" form-date" id="Email" placeholder="Village / Road" value="--><?php //=$Donor->getAddress2()?><!--" disabled>-->
<!--                    </div>-->
                    <div class="d-flex w-100 justify-content-center align-items-center ">
                        <label for="City" class="form-label w-40">City</label>
                        <input type="text" class=" form-date" id="City" placeholder="City" value="<?= $data['City']?>" disabled>
                    </div>
                </div>
            </div>
...
<!--                <button class="btn btn-danger m-1" style="padding: 0.5rem;font-size: 1rem" onclick="Cancel()">Cancel</button>-->
    ...
    <div class="d-flex flex-column justify-content-center align-items-center w-25 ">
        <div class="d-flex flex-center flex-column">
            ...
            <div class="d-flex flex-column m-1 px-2 py-0-5 text-center text-white border-radius-10 <?=$data['Donation_Availability']===1 ? "bg-success ": "bg-danger"?>">
                <?= $data['Donation_Availability'] === 1 ? "Available" : "Not Available"?>
            </div>
        </div>
        <div class="d-flex flex-column m-1">
            <img src="<?= $data['Profile_Image'];?>" alt="Avatar" class="avatar" width="192px">
...
<!--                TODO Donor NIC FRONT AND BACK-->
                <?php
                if($data['NIC_Front'] == null):
                ?>
                <img src="<?=Donor::DEFAULT_NIC_FRONT?>" alt="Avatar" class="avatar" width="200px" height="280px">

                <div class="text-md text-center d-flex flex-center gap-1 cursor my-1 font-bold border-radius-5 bg-dark text-white py-1" onclick="UploadNICFront('<?= $data['Donor_ID']?>')">
                ...
                ?>
                <img src="<?=$data['NIC_Front'];?>" alt="Avatar" class="avatar" width="200px" height="280px">
            ...
            <div class="d-flex flex-column">
                <?php
                if($data['NIC_Back'] == null):
                ?>
                <img src="<?=Donor::DEFAULT_NIC_BACK?>" alt="Avatar" class="avatar" width="200px" height="280px">
                    <div class="text-md d-flex flex-center cursor gap-1 text-center my-1 font-bold bg-dark border-radius-5 text-white py-1" onclick="UploadNICBack('<?=$data['Donor_ID']?>')">
                    <img src="/public/icons/camera.svg" class="invert-100">
                    Upload NIC <br> (Back)
                </div>
                <?php
                ...
                ?>
                <img src="<?=$data['NIC_Back'];?>"  alt="Avatar" class="avatar" width="200px" height="280px">
